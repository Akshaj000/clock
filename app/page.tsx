...
import { VideoCallDemo, WifiPasswordScreen } from './components/video-call';
...
export default function Home() {
  ...
  // Simulate connecting and always show glitch first, then wifi screen, then call
  useEffect(() => {
    if (!callStarted && !connectError && !showWifiScreen) {
      setGlitch(true);
  ...
  }, [callStarted, connectError, showWifiScreen]);
  ...
  const handleWifiPasswordSuccess = () => {
  ...
  return (
    <main className="min-h-screen w-full max-w-full flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800 p-0 sm:p-0 overflow-x-hidden">
      <div className="relative w-full max-w-full flex items-center justify-center">
        {connectError && !showWifiScreen ? (
          <div className="flex flex-col items-center justify-center gap-6">
            <div className="text-red-400 text-2xl font-bold animate-pulse">WiFi Disconnected</div>
            ...
            <div className="text-gray-300 text-lg">Network connection lost. Please reconnect WiFi.</div>
            <button
            ...
            <div className="text-gray-400 text-sm">WiFi authentication required</div>
          </div>
        ) : showWifiScreen ? (
          <WifiPasswordScreen
